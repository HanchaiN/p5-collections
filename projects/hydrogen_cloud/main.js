import { RADIUS_REDUCED } from "./psi.js";
import { getParentSize } from "../utils/index.js";

export default function execute() {
    let parent = null;
    let canvas = null;
    let resizeObserver = null;
    let worker;
    const states = [{
        coeff: { re: 1 }, psi: { n: 3, l: 2, m: -1 }
    }];
    const n_max = states.reduce(
        (n_max, { psi: { n } }) => Math.max(n_max, n),
        0
    ) + 2;
    let scale;

    function parentResized() {
        if (!canvas) return;
        const ctx = canvas.getContext("2d");
        const { width, height } = getParentSize(parent, canvas);
        ctx.canvas.width = width;
        ctx.canvas.height = height;
        scale = Math.pow(n_max, 2) * RADIUS_REDUCED / width;
        worker.postMessage({ width, height, scale });
    }
    function setup() {
        resizeObserver = new ResizeObserver(parentResized).observe(parent);
        canvas = document.createElement("canvas");
        const ctx = canvas.getContext("2d");
        const { width, height } = getParentSize(parent, canvas);
        ctx.canvas.width = width;
        ctx.canvas.height = height;
        parent.appendChild(canvas);
        scale = Math.pow(n_max, 2) * RADIUS_REDUCED / width;
        worker.postMessage({ width, height, states, scale, z_depth: 0, render: true });
        worker.addEventListener("message", function draw(e) {
            if (!e.data.buffer) return;
            const width = ctx.canvas.width, height = ctx.canvas.height;
            if (e.data.width === width && e.data.height === height) {
                const image = new ImageData(e.data.buffer, width, height);
                if (e.data.z_depth > scale * width)
                    worker?.postMessage?.({ render: true, z_depth: -scale * width });
                else
                    worker?.postMessage?.({ render: true, z_depth: e.data.z_depth + .5 * scale * width / 10 });
                ctx.putImageData(image, 0, 0, 0, 0, width, height);
            }
            ctx.strokeStyle = "blue"
            for (let i = 0; i <= n_max; i++) {
                ctx.beginPath();
                ctx.arc(width / 2, height / 2, Math.pow(i, 2) / (2 * scale), 0, 2 * Math.PI)
                ctx.stroke();
            }
        });
    }

    return {
        start: (node) => {
            parent = node;
            worker = new Worker(import.meta.resolve("./worker.js"), { type: "module" });
            setup();
        },
        stop: () => {
            canvas?.remove();
            resizeObserver?.disconnect();
            worker = parent = canvas = resizeObserver = null;
        },
    };
}
